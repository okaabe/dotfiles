...
local function create(c, is_tab_focused, buttons, is_client_inactive)
    ...
    return wibox.widget {
        {
            {
                {
                    {
                        markup = colorize_text(string.upper(c.class or "-"), colors.foreground),
                        align  = beautiful.tabbar_text_align or "left",
                        valign = beautiful.tabbar_text_align or "left",
	    ...
	    nil,
            {
                {
                    helpers.add_click_event(helpers.text("", colors.foreground), function()
                        c:kill()
                    end),
                    layout = wibox.layout.fixed.horizontal,
                },
                right = dpi(beautiful.tabbar_margin or dpi(5)) + 5,
                widget = wibox.container.margin,
            },
            expand = "none",
